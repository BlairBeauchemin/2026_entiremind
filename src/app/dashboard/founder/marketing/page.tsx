...
import {
  CampaignList,
  type CampaignView,
} from "@/components/dashboard/marketing/campaign-list";
...
type PieceWithMedia = ContentPieceRow & {
...
export default async function MarketingPage({
  ...
  if (brands.length === 0) {
    return (
      <div className="space-y-4">
        <h1 className="font-serif text-3xl text-ink">Marketing Engine</h1>
        <p className="text-muted-foreground">
          No brands found. Run migration 018_marketing_engine.sql to seed the
          Entiremind brand.
        </p>
      </div>
    );
  ...
  const draftCount = pieces.filter((p) =>
    ["draft", "generating"].includes(p.status),
  ).length;
  ...
  for (const row of (countRows ?? []) as Array<{
    ...
    pieceCountByCampaign.set(
      row.campaign_id,
      (pieceCountByCampaign.get(row.campaign_id) ?? 0) + 1,
    );
  }
  ...
  const { data: pieceCampaignRows } = await serviceSupabase
    ...
    .eq("brand_id", selectedBrand.id);
  for (const row of (pieceCampaignRows ?? []) as Array<{
    id: string;
    campaign_id: string | null;
  }>) {
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            ...
            <span className="text-ink font-medium">{selectedBrand.name}</span>.
            {draftCount > 0 &&
              ` ${draftCount} piece${draftCount === 1 ? "" : "s"} queued for generation.`}
          </p>
          <Link
      ...
      <div>
        <h2 className="font-serif text-2xl text-ink mb-2">Review Queue</h2>
        <p className="text-sm text-muted-foreground mb-4">
          Approve, edit, or reject before anything publishes. Paid ads always
          stop here.
        </p>
      ...
      <div>
        <h2 className="font-serif text-2xl text-ink mb-2">
          Awaiting Your Footage
        </h2>
        <p className="text-sm text-muted-foreground mb-4">
          Scripts ready for you to film. Upload the footage and the piece moves
          to review.
        </p>
      ...
      <div>
        <h2 className="font-serif text-2xl text-ink mb-2">Campaigns</h2>
        <p className="text-sm text-muted-foreground mb-4">
          &quot;Plan content now&quot; researches trends and drafts a week of
          pieces; the daily cron generates them one batch at a time.
        </p>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Approved content waits for the publish cron — or push it out
          immediately.
        </p>
      ...
      <div>
        <h2 className="font-serif text-2xl text-ink mb-2">Performance</h2>
        <p className="text-sm text-muted-foreground mb-4">
          Per-campaign totals from the daily metrics pull. Feeds back into next
          week&apos;s planning.
        </p>
      ...
      <div>
        <h2 className="font-serif text-2xl text-ink mb-2">Trend Research</h2>
        <p className="text-sm text-muted-foreground mb-4">
          Latest research snapshot feeding campaign planning. AI-sourced until
          live trend APIs are wired.
        </p>
        <TrendPanel snapshot={snapshotView} />
      </div>

      <div>
        <h2 className="font-serif text-2xl text-ink mb-2">Channels</h2>
        <p className="text-sm text-muted-foreground mb-4">
          Organic channels can auto-publish or require approval. Paid ads always
          require approval.
        </p>
        <ChannelSettings
