...
export function IntentionsTimeline({
  ...
  function getMessagesForIntention(
    ...
    const endDate = nextIntention
      ? new Date(nextIntention.createdAt)
      : new Date();
        ...
        return (
          ...
          >
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${
                    isActive ? "bg-emerald-500 animate-pulse-slow" : "bg-rule"
                  }`}
                />
                ...
                {isActive && (
                  <Link
                    href="/dashboard"
                    className="text-xs text-muted hover:text-cobalt underline-offset-2 hover:underline ml-2"
                  >
                    Edit
                  </Link>
                )}
            ...
            {intentionMessages.length > 0 && (
              <>
                <div className="h-px bg-cobalt-wash my-6" />
                      ...
                      >
                        <div
                          className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center ${
                            isOutbound ? "bg-cobalt-wash" : "bg-cobalt-wash"
                          }`}
                        >
                          {isOutbound ? (
                            <Bot
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm leading-relaxed ${
                              isOutbound ? "text-muted italic" : "text-muted"
                            }`}
                          >
                            {message.body}
                          </p>
